...
"""
# GLOBAL VARIABLES
global dic_ambs, complement, dic_kmerdg
...
def estimate_binding(sub_primer1, sub_primer2, type="kmer", dic_kmerdg={}):
    """
    ...
    ' ' not a complement between primers
    ...
    returns: [delta-G estimate, longest run of complement bases, alignment string]
    """
    dic_result = {}
    
    flt_currentest = 0; int_run = 0
    flt_est = 999; int_endpos = -1; int_maxrun = -1
    str_alignment = "" # will change with iteration

    for n in range(len(sub_primer1)):
        # check if it's a complement, if so, check if A-T or G-C
        if sub_primer1[n] == complement[sub_primer2[n]]:
            str_alignment += ":"
    ...
    # *remember that strings are immutable, so we can't just change the index
    # str_newalignment will change the strongest continuous run of complementary
    # nucleotides (similar to Thermofisher's algorithm)
    str_newalignment = str_alignment[:int_endpos-int_maxrun+1] \
        + "|"*int_maxrun + str_alignment[int_endpos+1:]
    str_printout = "5'-"+sub_primer1+"-3'\n" \
        +"   "+str_newalignment \
        +"\n3'-"+sub_primer2+"-5'\n"

    #jz243 - DELTA G LINEAR CORRELATION FUNCTION
    if type == "single-base":
        dic_result["dg"] = round(0.7616 * flt_est + 2.0863, 2)

    elif type == "2-mer":
        # get profile of 2-mers from primer1
        # the sub-sequence we're interested in should be at the same location as the pipes
        start = str_newalignment.find("|")
        end = str_newalignment.rfind("|")
        str_subseq1 = str(sub_primer1[start:end+1])
        ...
        for key in dic_kmerprofile:
            ...
            # the article had 10/16 possible kmers, so we created a neural network to predict the other 6
            # the NN algorithm is written in a jupyter notebook neural_network.ipynb
            flt_estimate -= dic_kmerdg[key] * dic_kmerprofile[key]
        
        # generated a regression line on excel, got the formula with R^2 = 0.9988 excluding one outlier,
        # where we did not obtain the correct alignment
        flt_estimate = 1.0089 * flt_estimate + 0.0137

    elif type == "2-mer staggered":
        start = str_newalignment.find("|")
        end = str_newalignment.rfind("|")
        str_subseq1 = sub_primer1[start:end+1]
        str_subseq2 = sub_primer2[start:end+1]

        dic_kmerprofile = {}
        flt_estimate = 0

        for n in range(len(str_subseq1) - 1):
            if n % 2 == 0: kmer = str_subseq1[n:n+2]
            else: kmer = str_subseq2[n:n+2]

            if kmer in dic_kmerprofile: dic_kmerprofile[kmer] += 1
            else: dic_kmerprofile[kmer] = 1

        for key in dic_kmerprofile:
            flt_estimate -= dic_kmerdg[key] * dic_kmerprofile[key]

    dic_result["dg"] = flt_estimate
    dic_result["maxrun"] = int_maxrun
    dic_result["printout"]  = str_printout
    dic_result["alignment"] = str_subseq1
    dic_result["est"] = flt_est

    return dic_result

def get_alignments(primer1, primer2, type="2-mer", dic_kmerdg={}):
    """
    ...
    other information returned with a list. Based on that, we can find the strongest
    possible configuration for each set of primers
    """
    dic_lowestresult = {"est":999, "dg":999}
    ...
    while p2 < len(primer1):
        ...
        dic_result = estimate_binding(sub_primer1, sub_primer2, type=type, dic_kmerdg=dic_kmerdg)
            ...
            with open(pth_alignfile, "wt") as wf_align, open(pth_outfile, "wt") as wf_out:
                ...
                for p1 in dic_allseqs:
                    ...
                    for p2 in dic_allseqs:
                        dic_dgresult = get_alignments(dic_allseqs[p1], dic_allseqs[p2][::-1], \
                            type=self.cmb_esttype.currentText(), dic_kmerdg=dic_kmerdg)
